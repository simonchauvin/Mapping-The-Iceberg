...
public class PositionMapping : MonoBehaviour {
	...
	void Update () {
		...
		if (data.Length > 0) {
			...
			direction = float.Parse(positions[positions.Length - 2].Split("+"[0])[1]);
			Vector3 pointToReach = new Vector3(float.Parse(lastPosition[0]) * factor, 1.0f, float.Parse(lastPosition[1].Split('+')[0]) * factor);
